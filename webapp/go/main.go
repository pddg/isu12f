...
var (
	dbIPs = []string{"133.152.6.250", "133.152.6.251", "133.152.6.252", "133.152.6.253"}
)
...
func main() {
	...
	adminAuthAPI.PUT("/admin/master", h.adminUpdateMasters)
...
func (h *Handler) GetDB(id int64) *sqlx.DB {
	return h.DBs[id%numDBs]
}
...
func (h *Handler) apiMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.GetDB(userID).Get(masterVersion, query); err != nil {
...
func (h *Handler) checkSessionMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		sessUserID, err := getUserIDFromSessID(sessID)
		if err != nil {
			return errorResponse(c, http.StatusUnauthorized, ErrUnauthorized)
		}
...
func (h *Handler) listGacha(c echo.Context) error {
	...
	err = h.GetDB(userID).Select(&gachaMasterList, query, requestAt, requestAt)
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	...
	for _, v := range gachaMasterList {
		...
		err = h.GetDB(userID).Select(&gachaItem, query, v.ID)
		if err != nil {
			return errorResponse(c, http.StatusInternalServerError, err)
		}
...
func (h *Handler) drawGacha(c echo.Context) error {
	...
	err = h.GetDB(userID).Select(&gachaItemList, "SELECT * FROM gacha_item_masters WHERE gacha_id=? ORDER BY id ASC", gachaID)
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	...
	// weightの合計値を算出
	var sum int64
	err = h.GetDB(userID).Get(&sum, "SELECT SUM(weight) FROM gacha_item_masters WHERE gacha_id=?", gachaID)
	if err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, err)
		}
...
func generateSessID(userID int64) (string, error) {
	id, err := generateUUID()
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("%s-%d", id, userID), nil
}

func getUserIDFromSessID(sess string) (int64, error) {
