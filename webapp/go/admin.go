...
func (h *Handler) adminSessionCheckMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.GetDB(0).Get(adminSession, query, sessID); err != nil {
		...
		if adminSession.ExpiredAt < requestAt {
			...
			if _, err = h.GetDB(0).Exec(query, requestAt, sessID); err != nil {
				return errorResponse(c, http.StatusInternalServerError, err)
			}
...
func (h *Handler) adminLogin(c echo.Context) error {
	...
	sessID, err := generateSessID(user.ID)
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	sess := &Session{
...
func (h *Handler) adminListMaster(c echo.Context) error {
	...
	if err := h.GetDB(0).Select(&masterVersions, "SELECT * FROM version_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	items := make([]*ItemMaster, 0)
	if err := h.GetDB(0).Select(&items, "SELECT * FROM item_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	gachas := make([]*GachaMaster, 0)
	if err := h.GetDB(0).Select(&gachas, "SELECT * FROM gacha_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	gachaItems := make([]*GachaItemMaster, 0)
	if err := h.GetDB(0).Select(&gachaItems, "SELECT * FROM gacha_item_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	presentAlls := make([]*PresentAllMaster, 0)
	if err := h.GetDB(0).Select(&presentAlls, "SELECT * FROM present_all_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)

	}

	loginBonuses := make([]*LoginBonusMaster, 0)
	if err := h.GetDB(0).Select(&loginBonuses, "SELECT * FROM login_bonus_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)

	}

	loginBonusRewards := make([]*LoginBonusRewardMaster, 0)
	if err := h.GetDB(0).Select(&loginBonusRewards, "SELECT * FROM login_bonus_reward_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
// PUT /admin/master
func (h *Handler) adminUpdateMasters(c echo.Context) error {
	var err error

	v := &VersionMaster{}
	for _, db := range h.DBs {
		v, err = h.adminUpdateMaster(db, c)
		if err != nil {
			return err
		}
	}
...
func (h *Handler) adminUpdateMaster(db *sqlx.DB, c echo.Context) (*VersionMaster, error) {
	tx, err := db.Beginx()
	if err != nil {
	...
	if err != nil {
		if err != ErrNoFormFile {
			return nil, errorResponse(c, http.StatusBadRequest, err)
		}
	}
...
func (h *Handler) adminBanUser(c echo.Context) error {
	...
	if _, err = h.GetDB(userID).Exec(query, banID, userID, requestAt, requestAt, requestAt); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
